...
class CSVPPTGenerator:
    ...
    def load_and_analyze_csv(self, csv_file_path: str) -> Dict[str, Any]:
        """Load CSV file and perform comprehensive analysis"""
        try:
    ...
    def generate_insights_with_ai(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        prompt = (
            ...
            '      "content": ["High-level finding (max 80 chars)", "Key recommendation (max 80 chars)"],\n'
            '      "slide_type": "overview"\n'
            '    },\n'
            '    {\n'
            '      "title": "Data Quality Assessment (max 50 chars)",\n'
            '      "content": ["Missing data insight (max 80 chars)", "Outlier finding (max 80 chars)"],\n'
            '      "slide_type": "insights"\n'
            '    }\n'
            '  ],\n'
            '  "recommended_charts": [\n'
        ...
        return {
            ...
            "slides": [
                ...
                {"title": "Dataset Overview", "content": [
                    f"📊 Records: {analysis['shape'][0]:,}",
                    f"📈 Columns: {analysis['shape'][1]}",
                    f"🔢 Numeric columns: {len(nums)}",
                    f"📝 Categorical columns: {len(cats)}",
                    f"📋 Charts generated: {len(chart_recommendations)}"
                ], "slide_type": "overview"}
            ],
